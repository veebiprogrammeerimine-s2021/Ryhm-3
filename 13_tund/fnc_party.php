<?php
	...
	function registered_data(){
        ...
        //SELECT id FROM vp_party WHERE cancelled IS NULL
        //$count_registered = 0;
        //while($stmt->fetch()){
        //  $count_registered ++;  //$count_registered += 1   count_registered = $count_registered + 1    
        //}
	...
	function forms_for_payment(){
		...
		while($stmt->fetch()){
			if(empty($payment_from_db)){
				...
				$list_html .= "<p>".$firstname_from_db ." " .$lastname_from_db .", MAKSNUD</p> \n";
			}
		}
		if(empty($list_html)){
